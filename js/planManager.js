...
async function savePlan(plan) {
  ...
  /**
   * Save a recent plan to local storage
   * @param {string} name - Account name
   * @param {string} id - Spreadsheet ID
   * @param {string} url - Spreadsheet URL
   * @param {string} gid - Sheet ID
   */
  async function saveRecentPlan(name, id, url, gid = null) {
    try {
      ...
      if (uniquePlans.length > MAX_RECENT_PLANS) {
        uniquePlans.splice(MAX_RECENT_PLANS);
      }
      
      await RevPilotStorage.setItem(STORAGE_KEY_RECENT_PLANS, uniquePlans);
      console.log("Plan saved successfully:", uniquePlans);
      
      // Show a success toast
      RevPilotUI.showToast('Plan saved successfully!', 'success');
      
      return uniquePlans;
    } catch (error) {
      console.error("Error saving recent plan:", error);
      throw error;
    }
  }
